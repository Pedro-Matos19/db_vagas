...
public class CompanyService {
	...
	@Transactional
	public void unableCompany(Long id) {
		Optional<Company> companyOpt = companyRepository.findById(id);

		Company company = companyOpt.orElseThrow(() -> new IdNotFoundException());
	...
	public void disableCompany(Long id) {
		...
		for (Job job : jobs) {
			job.setStatus(JobStatus.CANCELADA);
			...
			for (Application app : applications) {
				...
				interviews.forEach(i -> {
					if(i.getStatus() != InterviewStatus.REALIZADA) i.setStatus(InterviewStatus.CANCELADA);
				});
			}
		}
